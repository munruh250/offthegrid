...
public sealed class MoraleTests
{
    ...
    public void StartingMoraleScalesWithResolve()
    {
        ...
        // Currently 82 + 0.8*Resolve. Resolve was rescaled twice: halved (ratified
        // in balance doc 7.5), then trimmed again toward a 1.25 days-per-point
        // target. Measured at 1.39 against the next attribute's 1.08.
        Assert.Equal(82.8f, Fresh(resolve: 1).Current, 0.01f);
        Assert.Equal(86f, Fresh(resolve: 5).Current, 0.01f);
        Assert.Equal(90f, Fresh(resolve: 10).Current, 0.01f);
    ...
    public void ResolveBluntsMemoryEvents()
    {
        ...
        // [DEVIATES FROM SPEC 5.6] Divisor is 42, not 12 - see the note on
        // StartingMoraleScalesWithResolve. The property under test is unchanged.
        Assert.True(Math.Abs(strongHit) < Math.Abs(weakHit));
        Assert.Equal(-20f * (1f - 1f / 42f), weakHit, 0.01f);
        Assert.Equal(-20f * (1f - 10f / 42f), strongHit, 0.01f);
    }
